...
/* This function
 * copy of eig3volume.c written by D.Kroon University of Twente (May 2009)
 * modified by YC (25/09/2015)
 * [...]=eig3volume_YC(Ixx,Ixy,Ixz,Iyy,Iyz,Izz);
 *
 */
...
/* Note for this mex function eig3volume_YC
04/03/2016 Y.Chen
% ====================================================
	The output number can be 6 or 12,
	    If nb_OUT=6, they are Lambda1,Lambda2,Lambda3,Wx,Wy,Wz 
	    	    with abs(Lambda1)<abs(Lambda2)<abs(Lambda3) and [Wx,Wy,Wz] is eigenvector
				of Lambda3
	    If nb_OUT=12, they are Lambda1,Lambda2,Lambda3 and Vxyz, Uxyz, Wxyz
		    with abs of Lambda123 ascending order and V,U,W are eigenvectors
				correspoinding to Lambda1, Lambda2, Lambda3, respectively
% -----------------------------------------------------------------------------------------*/


void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
    double *Dxx, *Dxy, *Dxz, *Dyy, *Dyz, *Dzz;
    double *Dvec2x, *Dvec2y, *Dvec2z, *Deiga, *Deigb, *Deigc;
    double *Dvec1x, *Dvec1y, *Dvec1z;
    double *Dvec0x, *Dvec0y, *Dvec0z;

    float *Dxx_f, *Dxy_f, *Dxz_f, *Dyy_f, *Dyz_f, *Dzz_f;
    float *Dvec2x_f, *Dvec2y_f, *Dvec2z_f, *Deiga_f, *Deigb_f, *Deigc_f;
    float *Dvec1x_f, *Dvec1y_f, *Dvec1z_f;
    float *Dvec0x_f, *Dvec0y_f, *Dvec0z_f;
	...
	mexErrMsgTxt("3, 6 or 12 outputs are required");
    }
    ...
    if(mxGetClassID(prhs[0])==mxDOUBLE_CLASS) {
	  ...
	  /* Output eigenvalues in descending order */
        ...
        if(nlhs==6) {
            /* Output the eigenvector corresponding to the largest eigenvalue */
            ...
            Dvec2x = mxGetPr(plhs[3]); Dvec2y = mxGetPr(plhs[4]); Dvec2z = mxGetPr(plhs[5]);
	}
	else if(nlhs==12) {
	    /* Output all corresponding eigenvectors (ascending order of eigenvalues) */
            plhs[3] = mxCreateNumericArray(nsubs, idims, mxDOUBLE_CLASS, mxREAL);
            plhs[4] = mxCreateNumericArray(nsubs, idims, mxDOUBLE_CLASS, mxREAL);
            plhs[5] = mxCreateNumericArray(nsubs, idims, mxDOUBLE_CLASS, mxREAL);
            ...
            Dvec0x = mxGetPr(plhs[3]); Dvec0y = mxGetPr(plhs[4]); Dvec0z = mxGetPr(plhs[5]);
            Dvec1x = mxGetPr(plhs[6]); Dvec1y = mxGetPr(plhs[7]); Dvec1z = mxGetPr(plhs[8]);
            Dvec2x = mxGetPr(plhs[9]); Dvec2y = mxGetPr(plhs[10]); Dvec2z = mxGetPr(plhs[11]);
        }
       ...
       /* Assign pointers to each input. */
        ...
        if(nlhs==6) {
            ...
            Dvec2x_f  = (float *)mxGetPr(plhs[3]); 
            Dvec2y_f  = (float *)mxGetPr(plhs[4]); 
            Dvec2z_f  = (float *)mxGetPr(plhs[5]);
	}
	else if(nlhs==12) {
    	    /* Output all corresponding eigenvectors (descending order of eigenvalues) */
            plhs[3] = mxCreateNumericArray(nsubs, idims, mxSINGLE_CLASS, mxREAL);
            plhs[4] = mxCreateNumericArray(nsubs, idims, mxSINGLE_CLASS, mxREAL);
            plhs[5] = mxCreateNumericArray(nsubs, idims, mxSINGLE_CLASS, mxREAL);
            ...
            plhs[11] = mxCreateNumericArray(nsubs, idims, mxSINGLE_CLASS, mxREAL);            		    Dvec0x_f  = (float *)mxGetPr(plhs[3]); 
            Dvec0y_f  = (float *)mxGetPr(plhs[4]); 
            Dvec0z_f  = (float *)mxGetPr(plhs[5]);
	    Dvec1x_f  = (float *)mxGetPr(plhs[6]); 
            Dvec1y_f  = (float *)mxGetPr(plhs[7]); 
            Dvec1z_f  = (float *)mxGetPr(plhs[8]);
	    Dvec2x_f  = (float *)mxGetPr(plhs[9]); 
            Dvec2y_f  = (float *)mxGetPr(plhs[10]); 
            Dvec2z_f  = (float *)mxGetPr(plhs[11]);
        }
        ...
        for(i=0; i<npixels; i++) {
            ...
            if(nlhs==6) {
    	    	/* Output the eigenvector corresponding to the largest eigenvalue */
                Dvec2x_f[i]=(float)Davec[0][2]; 
                Dvec2y_f[i]=(float)Davec[1][2]; 
                Dvec2z_f[i]=(float)Davec[2][2];
	    }
	    else if(nlhs==12) {
    	    	/* Output all corresponding eigenvectors (descending order of eigenvalues) */
                Dvec0x_f[i]=(float)Davec[0][0]; 
                Dvec0y_f[i]=(float)Davec[1][0]; 
                Dvec0z_f[i]=(float)Davec[2][0];
                Dvec1x_f[i]=(float)Davec[0][1]; 
                Dvec1y_f[i]=(float)Davec[1][1]; 
                Dvec1z_f[i]=(float)Davec[2][1];
		Dvec2x_f[i]=(float)Davec[0][2]; 
                Dvec2y_f[i]=(float)Davec[1][2]; 
                Dvec2z_f[i]=(float)Davec[2][2];
            }
