function [Wav,WavS,S,dS,Vav,Gav_glob,Nav] = ...
         surfThickness(VxLst,VOL_QairI,config,lambda,nsub,offset)
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% %%         Measure the average thickness of a given 3D surface
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  [Wav,WavS,S,dS,Vav] = surfThickness(VxLst,VOL_Qair,lambda,config,binCapacity,nsub)
%  ----------------------------------------------------------------------------------
%
%   Inputs:
%   -------
...
%       >      offset: a positive value that will be added to the Qair
%                      profile to make sure no negative values appear in
%                      the profile 
%
%
%   Outputs:
%   --------
%       > Wav: average thickness of the surface (mean(Vav))
%       >WavS: average thickness of the surface (sum(Vav*dS)/S)
%       >   S: estimated surface area of the surface
%       >  dS: elementary surface area at each projected point
%       > Vav: elementary opening at each projected point
%       > Gav: elementary position (projected voxel position after fusion)
%       > Nav: elementary normal vector
%
%
%   written by Yang CHEN 12/07/2016
%   modified by Yang CHEN 03/06/2019
...
for i=1:npos_nor
   ...
   Z_nor(:,i) = inor(i).*VXYZ(:,3) + ptPosZ;
    %XYZ_nor(ipt,:) = inor.*VXYZ + ptPosXYZ(ipt,:);
...
%                 ---- modified 2019.06.03
%                      --> The effect of negative values in the Qair
%                          profile must be removed.
%                          To do so, an offset large enough is added to the
%                          whole profile, and the barycentre is computed
%                          from this modified Qair profile
% -------------------------------------------------------------------------
tmp = w_nor+offset;
inor0 = (tmp*inor') ./ sum(tmp,2);

X_median = inor0.*VXYZ(:,1)+ptPosX;
Y_median = inor0.*VXYZ(:,2)+ptPosY;
Z_median = inor0.*VXYZ(:,3)+ptPosZ;

clear X_nor Y_nor Z_nor voteId ptPos* wmax tmp
...
%                           Fuse the rebundant points into one single point
% -------------------------------------------------------------------------
